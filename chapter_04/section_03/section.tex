...
\alertTip{
    Figure \ref{fig:spiPinConnection} shows the regular mode setup. As the number of devices increases the
    number of $\overline{\texttt{CS}}$ lines needed also increases. That might become a problem. One way to
    solve this issue is to use a decoder to generate the $\overline{\texttt{CS}}$ lines. There is another
    way to solve this issue by using \textbf{daisy-chain}\footnote{Look up daisy-chaining SPI for more info.} method.
}
...
\begin{center}
    ...
    \end{tabularx}
    \captionof{table}{CPOL and CPHA states and corresponding clock polarity and edge selections for sampling and shifting data.}
    \label{tbl:spiModes}
\end{center}

\subsubsection{Data Frame}

The neat thing about SPI is that it does not define any standard data frame. This allows
the users to set the frame as we wish. This also means that interfacing\footnote{From the
data frame's perspective.} will be different to each slave devices.

\subsection{SPI: Serial Peripheral Interface of STM32H755ZI}

STM32H755ZI's has 6 SPI instances. And they supports full-duplex, half-duplex and simplex transactions.
It has multi-master mode and multi-slave mode capabilities. The SS\footnote{Slave Select.} can
be managed by both the hardware and software. Can configure SS signal polarity and timing. MISO and
MOSI swappable. Configurable polarity and phase using CPOL and CPHA bits. Configurable data order (choosing
whether to MSB or LSB first). SPI Motorola and TI formats supported. Hardware CRC feature.
...
\texttt{TODO: REST OF SPI}

\subsection{QUADSPI of STM32H755ZI}

Along with SPI, STM32H755ZI features a QUADSPI.

\alertNote{
    This QUADSPI can interface single-, dual-, or quad-SPI devices.
}
...
\begin{center}
    ...
    \end{tabularx}
    \captionof{table}{STM32H755ZI's QUADSPI modes.}
    \label{tbl:spiSTMModes}
\end{center}

\subsubsection{QUADSPI: Main Features}

\begin{itemize}
    \item 3 Functional modes: indirect, automatic static-polling, and memory-mapped modes.
    \item Dual-flash mode: 8 bits can be sent / received simultaneously by accessing two flash memories in parallel.
    \item SDR\footnote{Single Data Read.} and DDR\footnote{Double Data Read.} support.
    \item Fully programmable opcodes for both indirect and memory-mapped modes.
    \item Fully programmable frame format for both indirect and memory-mapped modes.
    \item 8-, 16-, 32-bit data access.
\end{itemize}

\subsubsection{QUADSPI Command Sequence: While }

\texttt{TODO: REST OF QUADSPI}
