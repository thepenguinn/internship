...
\ifSubfilesClassLoaded {
    \let\prechapter\chapter
    \let\presection\section
    \let\presubsection\subsection
    \let\presubsubsection\subsubsection
    \let\preparagraph\paragraph

    \let\paragraph\presubsubsection
    \let\subsubsection\presubsection
    \let\subsection\presection
    \let\section\prechapter


    \section{Camera Controller using STM32H745ZI} \label{sec:}
    % \gdef\subsubsection\subsection
    % \gdef\paragraph\subsubsection
    % \input{abstract.tex}
    % \thispagestyle{empty}
} {
...
At the heart of the camera controller, there will be an STM32H745ZI chip. The user input will be
taken using a joy stick and 6 push buttons. For debugging there will be a 3inch OLED display and
JTAG interface. The camera will be interfaced through UART.

\texttt{TODO: INSERT OVERVIEW IMAGE}
...
\subsection{Block Diagram of the Entire System}
...
\begin{center}
    \begin{tabularx} {\textwidth} {
        ...
        Device & Required Input Voltage Ranges & Driving Voltage \\ \midrule
        STM32H745ZI & $1.2\si{V}$ to $3.3\si{V}$ & $3.3\si{V}$ \\
        Joy Stick 67A-DF-IC-060C & $3.3\si{V}$ & $3.3\si{V}$ \\
        3inch OLED Display & $3.3\si{V}$ to $5\si{V}$ & $3.3\si{V}$ \\

        \bottomrule

    \end{tabularx}
    \captionof{table}{Required voltages and driving voltage of each components.}
\end{center}
...
\subsection{Interfacing Diagram: Joy Stick}
...
\subsubsection{Mechanical Guidelines: Joy Stick}
...
\begin{figure} [H]
    ...
    \includegraphics [
        ...
        max height = \IGXMaxHeight,
        \IGXDefaultOptionalArgs,
    ] {pics/joy_mech_guidelines.png}
    \captionof{figure} {Mechanical guidelines of Joy Stick.}
    \label{fig:joyMechGuide}
\end{figure}

\subsubsection{Mechanical Guidelines: OLED Module}
...
\begin{figure} [H]
    ...
    \includegraphics [
        ...
        max height = \IGXMaxHeight,
        \IGXDefaultOptionalArgs,
    ] {pics/oled_mech_guidelines.png}
    \captionof{figure} {Mechanical guidelines of OLED Module.}
    \label{fig:oledMechGuide}
\end{figure}
